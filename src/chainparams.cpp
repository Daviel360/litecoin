...
{
    const char* pszTimestamp = "Brandon is my best friend";
...
public:
    CMainParams() {
        strNetworkID = "main";
        consensus.nSubsidyHalvingInterval = HALVINGPERIOD;
        ...
        consensus.nPowTargetSpacing = BLOCKINTERVAL * 60;
        consensus.fPowAllowMinDifficultyBlocks = false;
        consensus.fPowNoRetargeting = false;
        consensus.nRuleChangeActivationThreshold = 6048; // 75% of 8064
        consensus.nMinerConfirmationWindow = 8064; // nPowTargetTimespan / nPowTargetSpacing * 4
        //consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].bit = 28;                                            //Lines 79-97 are commented out
        ...
        genesis = CreateGenesisBlock(GEN_Time, GEN_MAIN_Nonce, 0x1e0ffff0, 1, REWARD * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S(GEN_MAIN_Hash));
        ...
        vSeeds.emplace_back(SERVERIP_1);                        //my only defult node 
        //vSeeds.emplace_back("dnsseed.thrasher.io");
        //vSeeds.emplace_back("dnsseed.chocolatechiptools.com");
        //vSeeds.emplace_back("dnsseed.chocolatechippool.org");

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,ADDRESS_PREFIX);
        ...
        checkpointData = {
            {
                {  0, uint256S(GEN_MAIN_Hash)},
                //{  4032, uint256S("0x9ce90e427198fc0ef05e5905ce3503725b80e26afd35a987965fd7e3d9cf0846")},         //Check points were commented out 
...
public:
    CTestNetParams() {
        ...
        genesis = CreateGenesisBlock(GEN_Time, GEN_TEST_Nonce, 0x1e0ffff0, 1, REWARD * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S(GEN_TEST_Hash));
        assert(genesis.hashMerkleRoot == uint256S(GEN_Merkle));

        vFixedSeeds.clear();
        vSeeds.clear();
        // nodes with support for servicebits filtering should be at the top
        vSeeds.emplace_back(SERVERIP_1);
        //vSeeds.emplace_back("seed-b.chocolatechip.loshan.co.uk");
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
       ...
       // consensus.vDeployments[Consensus::DEPLOYMENT_CSV].bit = 0;
        ...
        genesis = CreateGenesisBlock(GEN_Time, 0, 0x207fffff, 1, REWARD * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S(GEN_REGTEST_Hash));
        assert(genesis.hashMerkleRoot == uint256S(GEN_Merkle));
