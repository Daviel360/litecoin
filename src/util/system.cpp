...
const char * const BITCOIN_CONF_FILENAME = "chocolatechip.conf";
...
#else
    const char* pszModule = "chocolatechip";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ChocolateChip";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ChocolateChip";
#else
    // Unix
    return pathRet / ".chocolatechip";
#endif
#endif
}
